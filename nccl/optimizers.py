...
class Optimizer(object):
    ...
    def _optimize_and_update(self, optimizer, **kwargs):
            ...
            for i in range(self.model.gpu_offset, self.model.num_gpus + self.model.gpu_offset):
                with tf.device('/gpu:' + str(i)):
                    with tf.variable_scope('gpu{}/gradients'.format(i)):
                        ...
                        var_list = tf.get_collection('dev_{}_trainable_variables'.format(i))
        ...
        if self.model.num_gpus == 1:
            ...
            with tf.variable_scope('calc/mean_gradients'):
                ...
                for grads_and_vars in zip(*tower_grads):
                    ...
                    grads, gvars = list(zip(*grads_and_vars))
                    grad_sums = nccl.all_sum(grads)
                    for g in grad_sums:
                        with tf.device(g.device):
                            g /= self.model.num_gpus
                        avg_grads.append(g)
                    all_avg_grads.append(avg_grads)
                    all_vars.append(gvars)

                avg_grads_and_vars = [(g, v) for (g, v) in zip(list(zip(*all_avg_grads)), list(zip(*all_vars)))]
                self.avg_grads = avg_grads_and_vars[0][0]
                apply_grads_ops = []
                with tf.control_dependencies(self.model.update_ops):
                    for n, grads_and_vars in enumerate(avg_grads_and_vars):
                        with tf.device('/gpu:{}'.format(n + self.model.gpu_offset)):
                            gv = list(zip(*grads_and_vars))
                            global_step = self.model.global_step if n == 0 else None
                            apply_grads_ops.append(optimizer.apply_gradients(gv, global_step=global_step))
        ...
        if weight_decay > 0.0:
            ...
            with tf.variable_scope('weight_decay'):
                ...
                with tf.control_dependencies(apply_grads_ops + self.update_ops):
                    decay_ops = []
            ...
            opt_op = tf.group(apply_grads_ops + self.update_ops)
        return opt_op
    ...
    def train(self, save_dir='./tmp', transfer_dir=None, details=False, verbose=True, show_each_step=False, **kwargs):
        ...
        if transfer_dir is not None:  # Transfer learning setup
            ...
            if blocks_to_load is None:
                for i in range(self.model.num_blocks):
                    var_list += tf.get_collection('block_{}_main_variables'.format(i))
                var_list += tf.get_collection('block_{}_main_variables'.format(None))
            else:
        ...
        saver = tf.train.Saver(var_list=tf.get_collection('main_variables'), max_to_keep=max_to_keep)
        ...
        with tf.device('/cpu:{}'.format(self.model.cpu_offset)):
            with tf.variable_scope('summaries'):  # TensorBoard summaries
                ...
                weights = weights[::self.model.num_gpus]
        ...
        self.model.session.run(tf.get_collection('var_assign_ops'))  # Assign main variables to device variables
        ...
        for i in range(num_steps - start_step):  # Training iterations
            ...
            if (i + 1) % validation_frequency == 0:  # Validation every validation_frequency iterations
                self.model.session.run(tf.get_collection('var_update_ops'))  # Update main variables before saving
                if self.val_set is not None:
