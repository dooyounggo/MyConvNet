        ...
        with tf.variable_scope('block_0'):
            with tf.variable_scope('conv_0'):
                x = self.conv_layer(X_input, kernels[0], strides[0], channels[0]//2, padding='SAME',
                                    biased=False, ws=True)
                print('block_0' + '/conv_0.shape', x.get_shape().as_list())
                d['block_0' + '/conv_0'] = x
                x = self.group_norm(x, num_groups=self.num_groups, shift=True, scale=True, scope='bn')
                d['block_0' + '/conv_0' + '/bn'] = x
                x = self.relu(x, name='relu')
                d['block_0' + '/conv_0' + '/relu'] = x
            with tf.variable_scope('conv_1'):
                x = self.conv_layer(x, kernels[0], 1, channels[0]//2, padding='SAME', biased=False, ws=True)
            ...
            with tf.variable_scope('conv_2'):
                x = self.conv_layer(x, kernels[0], 1, channels[0], padding='SAME', biased=False, ws=True)
