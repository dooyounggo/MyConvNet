...
class NADMNet(UnprocessingDemosaic):  # Noise-Adaptive DeMosaicing Network
    def _init_params(self, **kwargs):
        ...
        self.adascale_initializer = tf.initializers.variance_scaling(scale=1.0, mode='fan_in')
    ...
    def _build_model(self):
                    ...
                    if j > 0:
                        ...
                        if s > 1:
                            if self.striding_kernel_size is None:
                                ...
                                k = self.striding_kernel_size
                    if i == j == 0:
                        x = self.conv_unit(x, k, s, c, d, activation_type=self.activation_type, name=f'conv_{j}')
                    else:
                        x = self.mbconv_unit(x, k, s, c, m, d, activation_type=self.activation_type, name=f'mbconv_{j}')
        ...
        if self.strides[0] > 1:
            with tf.variable_scope('block_{}'.format(self._curr_block)):
                x = self.upsampling_2d_layer(x, scale=self.strides[0], upsampling_method='bilinear')
                features = self.conv_unit(x, self.kernels[0], 1, self.channels[0], d,
                                          activation_type=self.activation_type, name='conv_0')
        else:
            features = x
        ...
        with tf.variable_scope('block_{}'.format(self._curr_block)):
            ...
            with tf.variable_scope('conv_1'):
                init = tf.initializers.variance_scaling(scale=3.0, mode='fan_out')
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('conv_0'):
                x = self.conv_layer(x, 1, 1, out_channels*multiplier, padding='SAME',
                                    ...
                                    weight_initializer=tf.initializers.he_normal(), verbose=True)
                d[name + '/conv_0'] = x
                if self.use_bn:
                    x = self.batch_norm(x, shift=True, scale=True, scope='norm')
                x = self.activation(x, activation_type=activation_type)
    ...
    def adaptive_scaling(self, x, vector, scale=True, shift=True, name='adascale'):
        with tf.variable_scope(name):
            if self.channel_first:
                ...
                vector = vector[:, :, tf.newaxis, tf.newaxis]
            else:
                in_channels = x.get_shape().as_list()[-1]
                vector = vector[:, tf.newaxis, tf.newaxis, :]
