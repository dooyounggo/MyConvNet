...
def save_as_tfdata(subset_dir, destination_dir, copy=True):
    train_dir = os.path.join(subset_dir, 'Data', 'CLS-LOC', 'train')
    val_dir = os.path.join(subset_dir, 'Data', 'CLS-LOC', 'val')
    ...
    for i, (fname, info) in enumerate(zip(val_fnames, val_infos)):
        ...
        ext = img_dir.split('.')[-1]
        tree = emtree.parse(os.path.join(val_info_dir, info))
        root = tree.getroot()
        ...
        f = open(os.path.join(destination_dir, 'validation', '{:010d}.csv'.format(i)),
                 'w', encoding='utf-8', newline='')
        wrt = csv.writer(f)
        wrt.writerow([str(class_names.index(name))])
        f.close()
        i += 1
...
if __name__ == '__main__':
    subset_dir = "D:/Python/datasets/ILSVRC2017_CLS-LOC"
    destination_dir = "D:/Python/tfdatasets/ILSVRC2017_CLS-LOC"
